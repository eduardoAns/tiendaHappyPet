...
export const ProductCard: FC<Props> = ({ product }) => {
    ...
    const productImage = useMemo(() => {
        return isHovered
          ? `/products/${ product.images[1].src }`
          : `/products/${ product.images[0].src }`;
    ...
    return (
      ...
      >
          ...
          <Box sx={{ mt: 1, display: isImageLoaded ? 'block' : 'none' }} className='fadeIn'>              <Typography fontWeight={700}>{ product.title }</Typography>
